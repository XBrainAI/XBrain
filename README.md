...
**模板来源：** 类型 C 子站点（四季景点、生活点滴等）统一由仓库根 `src/` 共享模板库生成，模板文件、占位符令牌与引用规则见 `src/README.md`，禁止各子站本地另存模板副本。
...
部分子站点的 `index.html` 由 Python 脚本自动生成，**禁止手动编辑**，必须通过脚本重新生成。

#### 9.8.1 妈子站点（健康/妈）

| 项目 | 说明 |
|------|------|
| 生成脚本 | `健康/妈/generate_index.py` |
| 输出文件 | `健康/妈/index.html` |
| 数据源 | 目录下所有 `.md` 报告文件 + `个人健康档案与深度医学分析报告.md` |
| 排除文件 | `个人健康档案与深度医学分析报告.md`、`p.report.md`（非报告类） |
| 调用方式 | `cd 健康/妈 && python generate_index.py` |

**更新流程：**
1. 新增 `.md` 报告文件到 `健康/妈/` 目录
2. 更新 `个人健康档案与深度医学分析报告.md`（如需要）
3. 运行 `python generate_index.py` 重新生成 `index.html`
4. 推送代码到 Netlify

**禁止事项：**
- 禁止直接编辑 `index.html`（会被下次生成覆盖）
- 禁止重新编写生成逻辑（脚本已存在，直接调用即可）
- 禁止将 `p.report.md` 或 `MR-无需建立子站点/` 目录内容纳入生成

#### 9.8.2 识别脚本生成页面的方法

- 文件头部注释：`<!-- GENERATED BY generate_index.py - DO NOT EDIT -->`
- 目录中存在对应的 `generate_*.py` 脚本
- 文件体积异常大（通常 >100KB，因包含全量报告内容）

当需要更新此类页面时，AI 应：
1. 查找目录中的 `generate_*.py` 脚本
2. 直接执行脚本，而非重新编写生成逻辑
3. 如有新增排除文件需求，修改脚本中的 `EXCLUDE_FILES` 集合

---

## 十、文章页顶部导航（top-nav）标准模板

**适用范围**：仓库内所有**长文 / 文章页**（如「学习与成长 / 高考」科普文章、景点长图文等）。这类页面需要章节内跳转，应使用统一的顶部章节导航，替代原先「面包屑」式的无意义导航。

**标准模板文件**：`brand/top-nav-template.html`
- 这是一个**可直接浏览器打开预览**的演示页，包含完整交互（滚动高亮当前章节 + 手机端汉堡菜单）。
- 文件内用 `COPY BLOCK 1/3`、`2/3`、`3/3` 三段注释清晰标出需要复制到新页面的代码（CSS / HTML / JS），照抄即可。

**移植三步**：
1. **CSS**：把 `top-nav-template.html` 里 `COPY BLOCK 1/3`（变量与全局，若页面已有 `:root` 变量可省）、`2/3`（品牌 Logo + 顶部导航样式）、`3/3`（移动端媒体查询）三段复制进新页面 `<style>`。
2. **HTML**：复制品牌 Logo `<a class="xbrain-brand">` 与 `<nav class="top-nav">` + `<div class="nav-mobile-dropdown">` 两段到 `<body>` 顶部。
3. **JS**：复制 `</body>` 前的导航脚本（依赖 `id`：`topNav / navInner / navMobile / navToggle / hamburgerIcon`）。

**关键适配规则（务必遵守）**：
- **品牌 Logo 的 `href` 深度**按页面层级调整：
  - `子站点/index.html` → `../index.html`
  - `子站点/文章/index.html` → `../../index.html`
  - `子站点/分类/文章/index.html` → `../../../index.html`
  - 也可指向父级导航页（如 `../index.html` 或 `../../学习与成长/index.html`）。
- **章节锚点一一对应**：每条 `<a href="#id">` 必须在正文有 `id="id"` 的同名区块；首个区块加 `id="top"` 对应「顶部」。
- **桌面端与移动端导航项保持一致**：两者链接同步（桌面标签宜短，移动下拉可写全称）；新增章节时两处都要加。
- **正文顶部留白**：因顶栏固定（桌面 52px / 移动 48px），正文容器需预留 `padding-top`（文章页约 `7rem`，移动端 `6.5rem`），避免被遮挡。
- **可选「返回上一级」链接**：在 `nav-inner` 与 `nav-mobile-dropdown` 最前插入 `<a href="../index.html">← 返回子站</a>` 即可。
- **保持交互逻辑一致**：滚动高亮 `setActiveNav`、汉堡开合、点击页外自动收起，均无需改动，直接复用。

> 源模式来自「四季景点 / 花都周末家庭游」顶部导航，已统一为 XBrain 深色主题变量，作为全仓库长文页的标准。
