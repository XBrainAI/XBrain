...
## 13. 通用规则：营业时间核实 + 人文背景补充（攻略型子站必遵循）

> 本轮建设 `花都周末家庭游` 时新增的两条核心规则，适用于**所有攻略型子站**（多方案亲子游）。
> 深圳子站（§12）确立了"多方案切换架构"，本节在其之上补充两项硬性要求：**① 行程必须基于真实营业时间（禁止凭惯例填）**；**② 人文/历史类景点必须补面向孩子的背景解说**。
> 工作流默认顺序：先按 §11 出行偏好 + §12 架构搭骨架 → 再按 §13.1 联网核实营业时间 → 据此修订行程表（§13.2）→ 补人文背景（§13.5）。
